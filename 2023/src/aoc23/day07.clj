...
(defn compare-highest-cards-2 [a b]
  (let [comparison (compare (str/index-of cards2 (first a))
                            (str/index-of cards2 (first b)))]
    (if (= 0 comparison)
      (compare-highest-cards-2 (rest a) (rest b))
      comparison)))
...
(defn hand-type2 [hand]
  (let [joker-count (or ((frequencies hand) \J) 0)
        ...
        freqs (frequencies joker-update)]
    (cond
      (freqs 5) :five
      (freqs 4) :four
      (and (freqs 3) (freqs 2)) :full-house
